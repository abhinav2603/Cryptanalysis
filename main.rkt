...
(define (loop lst-subs acc)
  (if (null? lst-subs) acc
      (let ([dc-key (algo:dictionary-closure (car lst-subs))])
      (if (equal? #f dc-key) (loop (cdr lst-subs) acc)
          (let ([dc-swe-key (algo:secret-word-enumeration dc-key)])
            (if (equal? #f dc-swe-key) (loop (cdr lst-subs) acc)
                (begin (displayln dc-swe-key) (loop (cdr lst-subs) (append acc (list dc-swe-key)))))))))) 

(define (crack-cipher strategies key)
  (define (crack-cipher-helper strategies acc)
    (if (null? strategies) acc
        (let* ([strat1 (car strategies)]
               [list-subs (strat1 key)]
               [result (loop list-subs '() )])
        (if (null? result) (crack-cipher-helper (cdr strategies) (append acc (list key)))
            (crack-cipher-helper (cdr strategies) (append acc result))))))
  (crack-cipher-helper strategies '())) 
; (crack-cipher strat:composition key)
